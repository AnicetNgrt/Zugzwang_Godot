...
def get_player(player_id:int):
    ...
    return jsonify({ 'username': player.username }), 200, {'Location': url_for('get_player', player_id = player.id, _external = True)}
...
def ratelimit_handler(e):
    return make_response(
        jsonify({"reason":"Ratelimit exceeded %s" % e.description})
        , 429
    )
